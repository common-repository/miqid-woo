...
use MIQID\Plugin\WooCommerce\Includes\EConomic\DTO\{Currency, VatType};
...
class Currencies extends Base {
	 ...
	 * @param null $code
	 *
	 * @return Currency|Currency[]|HttpResponse
	 */
	function Get( $code = null ) {
		$HttpResponse = $this->RemoteGet(
			implode( DIRECTORY_SEPARATOR, array_filter( [ $this->GetEndpoint(), $code ] ) )
		);
		...
		if ( in_array( $HttpResponse->get_response_code(), [ 200 ] ) ) {
			...
			if ( isset( $result['collection'] ) ) {
				foreach ( $result['collection'] as $item ) {
					$Rows[] = new Currency( $item );
				}
			} else {
				$Rows[] = new Currency( $result );
			}
