...
use MIQID\Plugin\WooCommerce\Includes\EConomic\DTO\VatZone;

class VatZones extends Base {
	 ...
	 *
	 * @return VatZone|VatZone[]|HttpResponse
	 */
	function Get( $vatZoneNumber = null ) {
		$HttpResponse = $this->RemoteGet(
			implode( DIRECTORY_SEPARATOR, array_filter( [ $this->GetEndpoint( 'vat-zones' ), $vatZoneNumber ] ) )
		);
		...
		if ( in_array( $HttpResponse->get_response_code(), [ 200 ] ) ) {
			...
			if ( isset( $result['collection'] ) ) {
				foreach ( $result['collection'] as $item ) {
					$Rows[] = new VatZone( $item );
				}
			} else {
				$Rows[] = new VatZone( $result );
			}
